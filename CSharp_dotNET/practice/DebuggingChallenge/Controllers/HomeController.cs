...
public class HomeController : Controller
{
    ...
    //CreateUser was called MakeUser on the cshtml index page. updated index to match controller action name
    public IActionResult CreateUser(User newUser)
    {
        if(ModelState.IsValid)
        {
            ...
            //page should be called Generate, not Generator
    ...
    {
        //String is Username not just Name
    ...
    public void GeneratePassword()
    {
        ...
        for(int i = 1; i < 15; i++)
        {
            ...
            if(odds == 0)
            {
                password += CharOptions[rand.Next(CharOptions.Length)];
            } else {
                password += NumOptions[rand.Next(NumOptions.Length)];
            }
        }
